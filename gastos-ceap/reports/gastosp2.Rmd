...
gastos %>% 
  ...
  mutate(total_candidato = sum(valorLíquido)) %>% 
  ungroup() %>% 
  group_by(sgUF) %>% 
  mutate(total_estado = sum(valorLíquido),
         proporcao = (total_candidato / total_estado)*100) %>% 
  ...
  arrange(-proporcao) %>% 
  slice(1) %>% 
  ggplot(aes(x = reorder(nomeParlamentar, proporcao),
             y = proporcao,
             colour = regiao)) +
  geom_point(size = 6,
             alpha = .7) +
  geom_segment(aes(x = nomeParlamentar,
                   ...
                   y = 0,
                   yend = proporcao)) +
  geom_text(aes(label = paste(round(proporcao,1) ,"% (", sgUF, ")", sep = "")),
            color = "black",
            size = 2.5,
